...
public class Packets : MonoBehaviour
{
    ...
    private bool AsServer = false;       //per sapere se si sta inviando il pacchetto dal server
    private Settings settings;           //Per poter gestire gli errori e il debug
    private Lobby lobby;                 //Per poter sapere le informazioni sugli utenti connessi
    ...
    public void Send_Packet (string Richiedente,string Errore)
    {
        lock (this)
        {
            ...
            {
                settings.Error_Profiler("N011", 0, Richiedente + "(NetworkPacket => send_Packet): Socket excpetion: " + e.ErrorCode + "  " + Errore + e, 3, false);
                Risultato = false;
            }
            catch (Exception e)
            {
                settings.Error_Profiler("N011",0, Richiedente + "(NetworkPacket => send_Packet): excpetion: " + Errore + e,3,false);
                Risultato = false;
            }
        }
        if (Risultato == false)
    ...
    public void Receive_Packet (string Richiedente,int Dimension)
    {
        ...
        if (Risultato == false)
            return;
        settings.Console_Write(Richiedente + " (Receive) " + Body.Length + " Bytes ",false);
    }
    ...
    public byte First_Contact (byte Role,bool Received)
    {
        ...
        else  //Preparo per l'invio del pacchetto
        {
            ...
            if (Role >= 0 && Role <= 4)
            {
                //1: Connessione al server di un utente
                //2: Connessione per invio di dati
